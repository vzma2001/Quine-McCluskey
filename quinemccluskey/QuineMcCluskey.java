...
public class QuineMcCluskey {
	public static void main(String[] args) {
		...
		System.out.println(seperatedTerms.length);
		do {
			for (int i = 0; i < seperatedTerms.length; i++) {
				replaceDash(seperatedTerms[i], seperatedTerms, i);
			}
		} while (flag == false);
		for (int i = 0; i < seperatedTerms.length; i++) {
			do {
		...
		do {
			Boolean wasCombined = Simplify.combine(termList, longestCombo);
		...
		do {
			for (int i = 0; i < termList.size(); i++) {
				columnsArray = termList.get(i).getColumnsArray();
			...
			for (Integer j : implicantTable.keySet()) {
				for (int k = 0; k < implicantTable.get(j).size(); k++) {
					for (int l = 0; l < currentNums.size(); l++) {
						flag = true;
						if (currentNums.get(l) == implicantTable.get(k).get(j)) {
							flag = false;
						}
					}
					if (flag == true) {
						currentNums.add(implicantTable.get(k).get(j));
					}
					flag = true;
				}
			}
			...
			// Column Dominance
			// Make new hashmap, get rid of prime from combined list. usesd combined lsit
			// and put in map.
	...
	public static void replaceDash(String term, String[] seperatedTerms, int pos) {
		String branch1 = term.replaceFirst("-", "1");

		String branch0 = term.replaceFirst("-", "0");
		seperatedTerms[pos] = branch0;
		seperatedTerms = Arrays.copyOf(seperatedTerms, seperatedTerms.length + 1);
		seperatedTerms[seperatedTerms.length - 1] = branch1;
	}
